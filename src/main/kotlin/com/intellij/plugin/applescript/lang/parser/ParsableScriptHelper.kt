 ...
 *
 * The parser consumes this as one dictionary-query surface through [ParsableScriptSuiteRegistryHelper].
 * Splitting the interface would add facade glue without reducing the parser contract.
 *
...
interface ParsableScriptHelper {
     ...
     * the system dictionary registry before the parser queries them.
     * For performance reasons does NOT attempt to initialise an application whose name was not
     * discovered in standard paths at startup — e.g., the parser should not try to initialise an
     * application name that the user has not finished typing.
     *
