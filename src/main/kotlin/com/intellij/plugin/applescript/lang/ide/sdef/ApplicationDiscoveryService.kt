 ...
 * change, no extension-filter change. [com.intellij.plugin.applescript.test.parsing.ParserRegressionTest]'s
 * 6 fixtures regression-lock the discovery walk's outcome (every fixture references
 ...
 * `discoveredApplicationNames` updates do not need explicit synchronisation.
 *
...
class ApplicationDiscoveryService @JvmOverloads constructor(
     ...
     * discovered `.app` / `.osax` / `.sdef` bundle name into [discoveredApplicationNames].
     *
     ...
     * [APP_DEPTH_SEARCH], same [extensionSupported] filter excluding `xml`). ParserRegressionTest's
     * 6 fixtures regression-lock the resulting `discoveredApplicationNames` set.
     */
    ...
    /**
     * O(1) membership test on [discoveredApplicationNames]. Routed by the facade's
     ...
     */
    fun containsDiscoveredApplication(applicationName: String): Boolean =
        applicationName in discoveredApplicationNames
     ...
     */
    fun addDiscoveredApplicationName(applicationName: String): Boolean =
        discoveredApplicationNames.add(applicationName)
     ...
     */
    fun isInNotFoundList(applicationName: String): Boolean =
        applicationName in notFoundApplicationList
     ...
     * [AppleScriptSystemDictionaryRegistryService.createDictionaryInfoForApplication]
     * after a successful dictionary generation — once the application is known, it
     * should no longer be in the not-found memo.
     */
    fun removeFromNotFoundList(applicationName: String): Boolean =
        notFoundApplicationList.remove(applicationName)
     ...
     *
     * The notScriptable filter is applied after the walk (filtering `discoveredApplicationNames`
     * at consumer-side via [SdefPersistenceService.isNotScriptable]) rather than during
     * the walk itself — matches pre-Wave-3 behaviour byte-for-byte. ParserRegressionTest
     * 6 fixtures regression-lock the discovered name set verbatim.
     */
    ...
    companion object {
        ...
        // ParserRegressionTest's 6 fixtures regression-lock.
