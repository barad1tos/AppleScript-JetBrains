...
The plugin display name is standardized as `AppleScript` in both the source descriptor and Gradle-patched distribution metadata. This avoids the generic Marketplace template term "Support" and avoids JetBrains product-name terms such as "IDEA" while keeping the legacy plugin id unchanged for the listing-ownership decision above.
